...
vi.mock('@components', async (importOriginal) => {
  // Cast the module to an object so that spreading works
...
// Add helper function to wrap UI in MantineProvider
...
describe('Table', () => {
  ...
  describe('Rendering', () => {
    it('renders column headers', () => {
      ...
      // Updated expectation to match the actual rendered header texts
      expect(headerTexts).toEqual(
        expect.arrayContaining(['Description', 'Status']),
      );
    });
  ...
  describe('State handling', () => {
    ...
    it('shows error state', () => {
      ...
      renderWithProviders(<Table {...defaultProps} error={errorMessage} />);
    ...
    it('applies sorting when provided', () => {
      ...
      renderWithProviders(<Table {...defaultProps} sort={sort} />);
      // Assume Table renders each row with role "row"
  ...
  describe('TableKey integration', () => {
    ...
    it('handles large datasets', () => {
      ...
      // Expect one header row plus 10 data rows
    ...
    it('handles column with custom render function', () => {
      ...
      // Expect one instance per data row (3 rows)

...
type Store[T any] struct {
	...
	queryFunc          func(*output.RenderCtx, ...interface{}) error
...
func NewStore[T any](
	...
	s := &Store[T]{
		...
		storeType:   "sdk",
		contextKey:  contextKey, // Initialize
		state:       StateStale,
	}
...
// It now accepts variadic queryArgs to be passed to the underlying queryFunc.
func (s *Store[T]) Fetch(queryArgs ...interface{}) error { // Removed ctx *output.RenderCtx parameter
	...
	go func() {
		defer close(done)
		err := s.queryFunc(renderCtx, queryArgs...) // Use renderCtx from ContextManager
		if err != nil {
			errChan <- err
			return
		}
